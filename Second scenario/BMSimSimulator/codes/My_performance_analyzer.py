...
print(max_seq)
# saving source node,dest node, receiving time, sequence number, and generation time in the separated arrays  
for i in range(0, len(total), 5):
    source_node.append(total[i])

for i0 in range(1, len(total), 5):
    dist.append(total[i0])
...
while j < NUMBER_NODES:
    for i4 in range(len(source_node)):
        if source_node[i4] == j:
            node.append([source_node[i4],dist[i4] , generation_time[i4], receive_time[i4], seq_number[i4]])
    nodes.append(node)
    node = []
    j += 1
    
# give the unique value to the dist array
dist_num = list(Counter(dist).keys())
print("distances number:", dist_num)

for dis1 in dist_num:
    ...
    all_latency = []
    nodes_d=[]
    # find all nodes with the second element of the tuple equal to this distance
    for i in range(NUMBER_NODES):
        nodes_ds = []
        for j in range(len(nodes[i])):
            if nodes[i][j][1] == dis1:
                #nodes_d is the same format of nodes without 2nd element of the tuple (destination node)
                nodes_ds.append(nodes[i][j][0:1]+nodes[i][j][2:])
        nodes_d.append(nodes_ds)

    
    # calculating latency 
    for s in range(NUMBER_NODES):
        for d in range(len(nodes_d[s])):
            latency.append(nodes_d[s][d][2]-nodes_d[s][d][1])  # receive time - generation time
        if len(latency) == 0:
            latency.append(0)
    ...
    for s1 in range(NUMBER_NODES):
        for d1 in range(len(nodes_d[s1])):
            seq.append(nodes_d[s1][d1][3])
            if d1 >= (len((nodes_d[s1]))-1):
                burst_packet_loss.append(0)
            else:
                burst_packet_loss.append((nodes_d[s1][d1+1][3]-nodes_d[s1][d1][3])-1) # sequence number (n+1) - sequence number (n)
    ...
    plt.ylabel("PDR for destination "+str(dis1), fontsize=10)
    plt.savefig('PDR_'+str(dis1)+'.pdf', dpi=200)
    ...
    plt.ylabel("PDR (%) for destination "+str(dis1), fontsize=10)
    plt.savefig('PDR_boxplot_'+str(dis1)+'.pdf', dpi=200)
    ...
    plt.xlabel("Node Number", fontsize=10)
    plt.ylabel("Burst Packet Loss for destination "+str(dis1), fontsize=10)
    plt.savefig('burst_packet_loss_boxplot_'+str(dis1)+'.pdf', dpi=200)
    ...
    df23.plot(kind='bar')
    plt.xlabel("Node Number", fontsize=10)
    plt.ylabel("Mean Burst Packet Loss for destination "+str(dis1), fontsize=10)
    plt.savefig('mean_burst_packet_loss_'+str(dis1)+'.pdf', dpi=200)
    ...
    plt.xlabel("network", fontsize=10)
    plt.ylabel("latency (ms) for destination "+str(dis1), fontsize=10)
    plt.savefig('latency_boxplot_'+str(dis1)+'.pdf', dpi=200)
    ...
    plt.xlabel("Node Number", fontsize=10)
    plt.ylabel("Latency (ms) for destination "+str(dis1), fontsize=10)
    plt.savefig('latency_'+str(dis1)+'.pdf', dpi=200)

print("------------------------------------------")
print("nodes PDR ", nodes_PDR)
print("------------------------------------------")
print("average latency in each node for all destinations", all_latency_mean)

print("------------------------------------------")
print("average PDR in the whole network", network_PDR_mean)

print("------------------------------------------")
print("average burst packet loss in each node for all destinations", network_burst_packet_loss)
print("------------------------------------------")
